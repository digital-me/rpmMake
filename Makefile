...
RPM_CHANGES	?= - No information provided\n- Contact the packager
...
spec:
	...
	echo -n "Adding changelogs in the template spec file... ";
	test -f $(RPM_SPEC) || { echo "failed (see $(LOG_FILE))"; exit 1; };
	sed -i -r -e "/^%changelog/ a\$(RPM_CHANGES)\n" $(RPM_SPEC) >> $(LOG_FILE) 2>&1 || { echo "failed (see $(LOG_FILE))"; exit 1; };
	sed -i -r -e "/^%changelog/ a\* `date "+%a %b %d %Y"` $(RPM_PACKAGER) - $(RPM_VERSION)-$(RPM_RELEASE)" $(RPM_SPEC) >> $(LOG_FILE) 2>&1 || { echo "failed (see $(LOG_FILE))"; exit 1; };
	echo "ok";
